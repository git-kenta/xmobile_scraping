...
//--------------------------------------------------
function stock() {
    $num = show("select sum(stock_num) as 'all_num',sum(stock_web_num) as 'web_num' from stock");/*55/53*/
    $web = show("select count(plan_number) as 'web' from customer where plan_number = 2 and agreement = 1");
    $tenpo = show("select count(plan_number) as 'tenpo' from customer where plan_number = 2 and agreement = 2");
    
    $web_num = $num['web_num'] - $web['web'];
    $tenpo_num = ($num['all_num'] - $num['web_num']) - $tenpo['tenpo'];
    $list = [];
    $list['web_num'] = $web_num;
    $list['tenpo_num'] = $tenpo_num;
    return $list;
}
$stock = stock();
