...
if(isset($_GET['click_menu_agency'])) {    
    ...
    elseif($agency == 'agency_name') {
        if($_GET['number'] == 0) {
            echo '全体';
        } else {
            $name = show("select agency_name from agency where agency_id = {$_GET['number']}");
...
if(isset($_GET['delete_agency'])) {
    ...
    other_db("update customer set agency_id = 1 where agency_id = {$number}");
}
//限界突破WiFi在庫削除
