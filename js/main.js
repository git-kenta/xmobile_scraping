$(function () {
    ...
    $(document).on('click', '.menu-agency li', function () {
        ...
        $.ajax({
            ...
            data: {
                number: number,
                click_menu_agency: 'agency_name'
            }
        }).done(function (data) {
            $('.header-right-bottom h2 span').text(data);
        })
    ...
    $(document).on('click', '.menu_list .edit', function () {
        if ($(this).text() == '編集') {
            ...
            if ($(this).parents('.menu_list').find('.show').data('id') != 1) {
                $(this).parents('.menu_list').find('.delete_check').show();
            }
            ...
            $(this).parents('.menu_list').find('.delete_check').hide();
        }
    ...
    $(window).resize(function () {
        var w = $(window).width();
        if (w >= 1000) {
            $('.menu,.menu-agency,.menu-incentive,.menu-config').show();
        } else {
            $('.menu,.menu-agency,.menu-incentive,.menu-config').hide();
        }
    });
